...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.post_embed_cnn = nn.Conv1d(in_channels=embed_size, out_channels=hidden_size, kernel_size=2,
                                        padding='same')  # 这个非常关键，必须保证input和output相同的形状
        self.encoder = nn.LSTM(input_size=hidden_size, hidden_size=hidden_size, num_layers=1, bias=True,
                               bidirectional=True)
        # 对于hidden_size这个参数，当bidirectional=True时，hidden_size不需要手动乘2,i.e.hidden_size=2 * hidden_size
        ...
        # in_features=len(self.vocab.tgt)不是很确定呢
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # print(f"source_padded的形状: {source_padded.shape},分别为[句子长度,batch_size]")
        # print(f"输入句子的原始长度，列表长度=句子个数(batch_size)：{source_lengths}")

        X = self.model_embeddings.source(source_padded)
        # print(f"卷积前X的形状为：{X.shape}，分别为[句子长度,句子个数(batch_size),嵌入长度]")
        X = torch.permute(X, (1, 2, 0))
        X = self.post_embed_cnn(X)
        X = torch.permute(X, (2, 0, 1))
        # print(f"卷积后X的形状为：{X.shape}，分别为[句子长度,句子个数(batch_size),隐含层长度]")
        X = nn.utils.rnn.pack_padded_sequence(input=X, lengths=source_lengths, enforce_sorted=True)

        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)
        # print(f"last_hidden重组前的形状: {last_hidden.shape},分别为[2（对应正方向和反方向）,句子个数(batch_size),隐含层长度]")
        # print(f"last_cell重组前的形状: {last_cell.shape},分别为[2（对应正方向和反方向）,句子个数(batch_size),隐含层长度]")
        enc_hiddens, _ = nn.utils.rnn.pad_packed_sequence(enc_hiddens)

        # print(f"{enc_hiddens.shape}分别为[句子长度,句子个数(batch_size),嵌入长度*2（因为是双方向的）]")

        enc_hiddens = torch.permute(enc_hiddens, (1, 0, 2))

        last_hidden = torch.cat((last_hidden[0], last_hidden[1]), dim=1)
        last_cell = torch.cat((last_cell[0], last_cell[1]), dim=1)  # 这到底是几哇？1和-1形状都对——-1和1等效
        # print(f"last_hidden的形状: {last_hidden.shape},分别为[句子个数(batch_size),隐含层长度*2]")
        # print(f"last_cell的形状: {last_cell.shape},分别为[句子个数(batch_size),隐含层长度*2]")
        # print(last_hidden)
        # print(last_cell)

        init_decoder_hidden = self.h_projection(last_hidden)
        init_decoder_cell = self.c_projection(last_cell)
        # print(f"init_decoder_hidden的形状: {init_decoder_hidden.shape},分别为[句子长度,batch_size]")
        # print(f"init_decoder_cell的形状: {init_decoder_cell.shape},分别为[句子长度,batch_size]")
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, split_size_or_sections=1, dim=0):
            ...
            # print(f"Y_t的形状为：{Y_t.shape}")
            Y_bart = torch.cat((Y_t, o_prev), dim=1)
            dec_state, o_t, _ = self.step(Y_bart, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            combined_outputs.append(o_t)
            o_prev = o_t

        combined_outputs = torch.stack([combined_output for combined_output in combined_outputs], dim=0)
        ### END YOUR CODE
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # print(f"Ybar_t的形状为：{Ybar_t.shape}，分别为[句子个数(batch_size),embedding_size+hidden_size]")
        # print(f"dec_state[0]hidden_state的形状为：{dec_state[0].shape}，分别为[句子个数(batch_size),隐藏层个数]")
        # print(f"dec_state[1]cell_state的形状为：{dec_state[1].shape}，分别为[句子个数(batch_size),隐藏层个数]")
        dec_state = self.decoder(Ybar_t, (dec_state[0], dec_state[1]))
        dec_hidden, dec_cell = dec_state[0], dec_state[1]
        # print(f"enc_hiddens_proj的形状为：{enc_hiddens_proj.shape}，分别为[句子个数(batch_size),句子长度,隐藏层个数]")
        e_t = torch.bmm(torch.unsqueeze(dec_hidden, dim=1), torch.permute(enc_hiddens_proj, (0, 2, 1)))
        # dec_hidden(b,h)->(b,1,h)
        # attention scores e_t 但在这里e_t的形状为(b, 1, src_len)
        e_t = torch.squeeze(e_t, dim=1)  # 我的遗漏，这里的squeeze把batch_size的维度压缩了，恰好运行时batch_size=1，需要指定压缩的维度
        ...
        a_t = torch.squeeze(torch.bmm(torch.unsqueeze(alpha_t, dim=1), enc_hiddens), dim=1)  # alpha_t->(b, 1, src_len)
        U_t = torch.cat((dec_hidden, a_t), dim=1)  # dec_hidden(b,h) a_t(1,h)
